"""The dataset is the source of truth for what the salon sells, and for what it buys.
...
from aziza_adk import catalog_data, queries, supplies_data
...
_KEEP_SUPPLIES = [row["supply_ref"] for row in supplies_data.SUPPLIES]
...
def test_a_product_absent_from_the_dataset_is_retired(conn, stocked):
    queries.retire_absent(conn, _KEEP_PRODUCTS, _KEEP_SERVICES, _KEEP_SUPPLIES)
...
def test_a_supply_absent_from_the_dataset_stops_being_matched(conn):
    """The same rule over what the salon BUYS. A supply left active after the dataset drops it
    still groups reports under a name nobody chose (§16)."""
    gone = "sup-retired-sentinel"
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO supplies (supply_ref, name, aliases) VALUES (%(r)s, %(r)s, '') "
            "ON CONFLICT (supply_ref) DO UPDATE SET active = TRUE",
            {"r": gone},
        )
    try:
        queries.retire_absent(conn, _KEEP_PRODUCTS, _KEEP_SERVICES, _KEEP_SUPPLIES)
        assert gone not in {row["supply_ref"] for row in queries.supply_catalog(conn)}
    finally:
        with conn.cursor() as cur:
            cur.execute("DELETE FROM supplies WHERE supply_ref = %(r)s", {"r": gone})
