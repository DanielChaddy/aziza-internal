...
1. The specialist names a client and what she had done. Call `start_ticket` with the client's
   name, then `add_service` once per service.
...
def _catalog() -> list[str]:
    """Service names, cached per process. Empty when the database is unreachable — the block is
    then omitted rather than the turn failing over prompt context."""
    global _CATALOG_CACHE
    if _CATALOG_CACHE is None:
        try:
            with queries.connect() as conn:
                _CATALOG_CACHE = list(catalog.names(queries.service_catalog(conn)))
